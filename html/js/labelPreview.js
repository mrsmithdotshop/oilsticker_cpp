window.addEventListener('DOMContentLoaded', () => {
    ...
    function updateCalculatedFields() {
        ...
        document.getElementById('today').value = `${pad2(today.getMonth()+1)}/${pad2(today.getDate())}/${String(today.getFullYear()).slice(-2)}`;
        ...
        document.getElementById('formattedMileage').value = (mileage + nextService).toLocaleString();
    ...
    function drawLabel() {
        ...
        ctx.font = "15px Arial";
        const padding = 25;
        ctx.fillText(oilType, x + padding, y + 285*size/406 - 20);
        ctx.fillText(todayText, x + size - padding - ctx.measureText(todayText).width, y + 285*size/406 - 20);

        ctx.font = "30px Arial";
        ctx.fillText(formattedMileage, x + padding, y + 365*size/406 - 25);
        ctx.fillText(nextDate, x + size - padding - ctx.measureText(nextDate).width, y + 365*size/406 - 25);
    }
	...
	^FN2^FD${oilType}^FS
	^FN3^FD${today}^FS
	^FN4^FD${formattedMileage}^FS
	^FN5^FD${nextDate}^FS
	^XZ
        `;
        selectedDevice.send(zpl, undefined, err => { if(err) alert("Printer Error: " + err); });
    });
